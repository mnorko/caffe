...
loc_digit = np.array([0,20]) # box x location, box y location
imgTemp[loc_digit[1]:loc_digit[1]+28,loc_digit[0]:loc_digit[0]+28] =transformed_image[0,:,:]
...
bbox_x = 2*(-(width_input-bbox_size_x)/2 + loc_digit[0])/width_input
bbox_y = 2*(-(height_input-bbox_size_y)/2 + loc_digit[1])/height_input
theta = np.array([[scale_x,0,bbox_x],[0,scale_y,bbox_y]])
#theta = np.array([1,0,0,0,1,0])


net.blobs['data'].data[...] = imgUse
net.blobs['theta'].data[...] = theta.reshape(-1)
lmdb_cursor.next_nodup()
value = lmdb_cursor.value()
datum.ParseFromString(value)
label = datum.label
