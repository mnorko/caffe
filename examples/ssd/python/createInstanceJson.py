...
# Create cocoText instance json file that can be used for finding AP
import sys
...
#with open('/Users/marissac/caffe/examples/ssd/python/detection_cocoText_anns_' + repr(numImg) + '_legible.pickle','rb') as f1:
...
#with open('/Users/marissac/caffe/examples/ssd/python/detection_cocoText_imgToAnns_' + repr(numImg) + '_legible.pickle','rb') as f2:
...
for i in range(0,numAnns):
    #area = float(anns_start[i]['area'])
    #bbox_use = list(anns_start[i]['bbox'])
    ...
    #score = json.JSONEncoder(anns_start[i]['score'])
    #score = json.loads(repr(anns_start[i]['score']),parse_float =Decimal)
...
#img_anno = dict()
#img_anno["annotations"] = anno_img
#img_anno["categories"] = categories
