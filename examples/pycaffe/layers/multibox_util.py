...
def encodeBBoxes(prior_bboxes,prior_variances,gt_bboxes,match_gt_indices,match_indices):
	...
	Outputs:
	--------
	encode_bbox: contains encoded ground truth bounding boxes [max_matchesx4] where each set of four represents xmin, ymin, xmax, ymax
	"""
	max_matches = match_gt_indices.shape[0]
	for k in range(0,max_matches):
		prior_width = prior_bboxes[match_indices[k]]['xmax']-prior_bboxes[match_indices[k]]['xmin']
		prior_height = prior_bboxes[match_indices[k]]['ymax']-prior_bboxes[match_indices[k]]['ymin']
		prior_center_x = (prior_bboxes[match_indices[k]]['xmax']+prior_bboxes[match_indices[k]]['xmin'])/2.0
		prior_center_y = (prior_bboxes[match_indices[k]]['ymax']+prior_bboxes[match_indices[k]]['ymin'])/2.0

		bbox_width = gt_bboxes[match_gt_indices[k]]['xmax'] - gt_bboxes[match_gt_indices[k]]['xmin'] 
		bbox_height = gt_bboxes[match_gt_indices[k]]['ymax'] - gt_bboxes[match_gt_indices[k]]['ymin'] 
		bbox_center_x = (gt_bboxes[match_gt_indices[k]]['xmax'] + gt_bboxes[match_gt_indices[k]]['xmin'])/2.0
		bbox_center_y = (gt_bboxes[match_gt_indices[k]]['ymax'] + gt_bboxes[match_gt_indices[k]]['ymin'])/2.0

		xmin = (bbox_center_x-prior_center_x)/prior_width/prior_variances[match_indices[k]]['xmin']
		ymin = (bbox_center_y-prior_center_y)/prior_height/prior_variances[match_indices[k]]['ymin']
		xmax = np.log(bbox_width/prior_width)/prior_variances[match_indices[k]]['xmax']
		ymax = np.log(bbox_height/prior_height)/prior_variances[match_indices[k]]['ymax']

		encode_add = np.array([[xmin],[ymin],[xmax],[ymax]])
		if k == 0:
			encode_bbox = encode_add
		else:
			encode_bbox = np.concatenate((encode_bbox,encode_add))
	return encode_bbox
...
def createBBoxMatchSet(gt_bboxes,pred_bboxes,confidence,overlap_threshold,max_matches):
	...
	if num_new_matches >= num_add_matches:
		...
		match_indices = np.concatenate((match_indices,new_matches_idx,np.zeros((num_add_matches-num_new_matches))),axis = 0)
		match_gt_indices = np.concatenate((match_gt_indices,new_matches_gt_idx,np.zeros((num_add_matches-num_new_matches))),axis = 0)
...
def subselectBBoxes(bboxes,bbox_idx):
	...
	for k in range(0,max_matches):
		bbox_out[0,k] = bboxes[bbox_idx[k]]['xmin']
		bbox_out[1,k] = bboxes[bbox_idx[k]]['ymin']
		bbox_out[2,k] = bboxes[bbox_idx[k]]['xmax']
		bbox_out[3,k] = bboxes[bbox_idx[k]]['ymax']
	return bbox_out
